...
for dt = 1:nt
    ...
    rho(i,dt+1) = rho(i,dt) - c1*(rho(i+1,dt)-rho(i-1,dt)) + c2*(rho(i-1,dt) - 2*rho(i,dt) + rho(i+1,dt)) + c3*randn;
    % Doubling Boundary Conditions
    rho(x==1,dt+1) = 2*rho(end,dt);
    rho(end,dt+1) = rho(end,dt) - c1*(rho(x==1,dt)-rho(end-1,dt)) + c2*(rho(end-1,dt) - 2*rho(end,dt) + rho(x==1,dt));
    % Zero below x=1
    rho(x < 1, dt+1) = 0;
end
